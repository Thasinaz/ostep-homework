...
void print_usage(char *s) { fprintf(stderr, "Usage: %s -<n> file", s); }
...
void print_lines(int fd, size_t begin, size_t size) {
  ...
  size_t new_begin =
      begin < counter * exp_line_len ? 0 : begin - counter * exp_line_len;
...
int main(int argc, char *argv[]) {
  ...
  size_t begin = file_size < counter * exp_line_len
                     ? 0
                     : file_size - counter * exp_line_len;
